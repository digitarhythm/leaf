...
use crate::js_interop::{preview_search, preview_search_goto, preview_search_clear};

/// プレビュー（レンダリング済みHTML）内を検索する浮動バー。
/// Ace の検索ボックスはプレビューでは使えないため独自に実装している。
#[derive(Properties, PartialEq)]
pub struct PreviewSearchProps {
...
#[function_component(PreviewSearchBar)]
pub fn preview_search_bar(props: &PreviewSearchProps) -> Html {
        ...
        use_effect_with((q, props.match_case), move |(q, match_case)| {
            ...
            let timeout = Timeout::new(150, move || {
                if q.is_empty() {
                    ...
                    let n = preview_search(&q, match_case);
                    total.set(n);
    ...
    let go_next = {
        ...
        Callback::from(move |_: ()| {
            if *total > 0 {
                let i = preview_search_goto(*current + 1);
                current.set(i);
            }
        })
    };
    ...
    let counter = if no_match {
        i18n::t("preview_search_no_match", lang)
    } else if *total > 0 {
    ...
    html! {
        <div class="absolute top-3 right-4 z-30 flex items-center gap-2 bg-gray-900/95 border border-emerald-500/70 rounded-lg shadow-2xl px-3 py-2 backdrop-blur-sm"
            ...
            <input
                ...
                oninput={oninput}
                onkeydown={onkeydown}
                placeholder={i18n::t("preview_search_placeholder", lang)}
                spellcheck="false"
                autocomplete="off"
                class={classes!(
            ...
            <button
                ...
                title={i18n::t("preview_search_match_case", lang)}
                aria-label={i18n::t("preview_search_match_case", lang)}
                class={classes!(
            ...
            <button
                ...
                disabled={*total == 0}
                title={i18n::t("preview_search_prev", lang)}
                aria-label={i18n::t("preview_search_prev", lang)}
                class={btn_class}
            >
            ...
            <button
                ...
                disabled={*total == 0}
                title={i18n::t("preview_search_next", lang)}
                aria-label={i18n::t("preview_search_next", lang)}
                class={btn_class}
            >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
