...
pub fn app() -> Html {
    ...
    // Timers & Title
    {
        ...
        let is_init = is_initial_load.clone();
        ...
        use_effect_with((), move |_| {
            ...
            let callback = Closure::wrap(Box::new(move |_token: String| {
                if !*is_auth_cb {
                    ...
                    let s_state_inner = s_state_cb.clone();
                    let r_s_inner = r_s_cb.clone();
                    spawn_local(async move {
    ...
    // Global Key Events (ESC handling for modals)
    {
        ...
        let conflicts = conflict_queue.clone();
        let fallbacks = fallback_queue.clone();
        let sp = is_suppressing_changes.clone();
    ...
    let network_connected_cb = network_connected.clone();
    let is_ld_cb = is_loading.clone();
    let is_fo_cb = is_fading_out.clone();
    let r_is_saving_cb = is_saving_ref.clone();
    let sp_cb = is_suppressing_changes.clone();
    let lmk_cb = loading_message_key.clone();
    let f_queue_cb = fallback_queue.clone();
    ...
    let on_save_cb = {
        ...
        Callback::from(move |is_manual: bool| {
            if *r_is_saving_cb.borrow() { return; }
            ...
            if let Some(id) = aid_opt {
                ...
                let is_online = *network_connected_cb && web_sys::window().unwrap().navigator().on_line();
                ...
                if let Some(sheet) = sheet_opt {
                    ...
                    let s_clone = sheet.clone(); let s_inner = s_state.clone(); let nc_inner = network_connected_cb.clone();
                    let ild_inner = is_ld_cb.clone(); let ifo_inner = is_fo_cb.clone(); let ris_inner = r_is_saving_cb.clone(); let sp_inner = sp_cb.clone();
                    let lmk_inner = lmk_cb.clone(); let f_queue_inner = f_queue_cb.clone(); let r_ncid_inner = r_ncid.clone(); let r_s_async = r_s_cb.clone();
    ...
    let conflict_checked = use_state(|| false);
    {
        ...
        use_effect_with((is_auth, ncid.clone(), sheets.len(), (*aid).clone(), db_ready), move |deps| {
            let (auth, folder, _count, _, ready) = deps;
            if *auth && folder.is_some() && !*checked && *ready {
                ...
                spawn_local(async move {
                    ...
                    for (idx, s) in s_val.iter().enumerate() {
                                    ...
                                    if is_trashed {
                                        ...
                                        if let Ok(tv) = js_sys::Reflect::get(&meta, &wasm_bindgen::JsValue::from_str("modifiedTime")) {
                                            if let Some(ts) = tv.as_string() {
                                                ...
                                                if dt > lt && (dt - lt) > 2000 {
                                                    if let Ok(drive_val) = crate::drive_interop::download_file(did, None, None).await {
                        ...
                        } else if s.last_sync_timestamp.is_some() && !s.content.is_empty() {
                    ...
                    if !conflicts.is_empty() { q.set(conflicts); }
                    else { 
                        let finish = {
                            let ild = ild.clone(); let ifo_c = ifo_c.clone(); let is_init_inner = is_init_c.clone();
                            move || { ifo_c.set(true); Timeout::new(300, move || { ild.set(false); is_init_inner.set(false); }).forget(); }
                        }; 
            ...
            if **ready_val && !**ld_val { 
                if let Some(id) = &**aid_val { 
                    if let Some(s) = s_val.iter().find(|x| x.id == *id) { 
                        gloo::console::log!("[Sync] Initializing editor content from state.");
    ...
    let on_file_sel = {
        ...
        let r_s = sheets_ref.clone(); let sp = is_suppressing_changes.clone();
        Callback::from(move |(did, title, cat_id): (String, String, String)| {
            iv.set(false); il.set(true); ifo.set(false);
            let s_state = s_s.clone(); let aid_state = aid.clone(); let sp_inner = sp.clone();
            let il_inner = il.clone(); let ifo_inner = ifo.clone(); let rs = r_s.clone();
            spawn_local(async move {
                if let Ok(cv) = crate::drive_interop::download_file(&did, None, None).await {
                    if let Some(c) = cv.as_string() {
                        ...
                        *rs.borrow_mut() = cs.clone(); s_state.set(cs); aid_state.set(Some(nid.clone()));
                        set_editor_content(&c); set_gutter_status(false); focus_editor();
    ...
    let on_conf_cfm = {
        let cq = conflict_queue.clone(); let s_s = sheets.clone(); let il = is_loading.clone();
        ...
        Callback::from(move |sel: usize| {
            ...
            spawn_local(async move {
                let mut us = (*ss).clone(); let mut is_deleted = false;
                if let Some(pos) = us.iter().position(|x| x.id == conf.sheet_id) {
                    let s = &mut us[pos];
                    match sel {
                        0 => { if let Ok(dv) = crate::drive_interop::download_file(&conf.drive_id, None, None).await { if let Some(t) = dv.as_string() { s.content = t.clone(); s.temp_content = None; s.temp_timestamp = None; s.last_sync_timestamp = Some(conf.drive_time); s.is_modified = false; if Some(s.id.clone()) == aid_v { set_editor_content(&t); } } } },
                        1 => { 
                ...
                if us.is_empty() {
                    ...
                    let nid = us.last().unwrap().id.clone(); aid_inner.set(Some(nid));
                }
                ss.set(us.clone()); qs.set(q.clone());
                if q.is_empty() { 
                    ifod.set(true); let ild_final = ild_final_c.clone(); let aid_final = aid_final_c.clone(); let us_final = us.clone();
                    Timeout::new(350, move || { ild_final.set(false); if let Some(id) = (*aid_final).clone() { if let Some(s) = us_final.iter().find(|x| x.id == id) { set_editor_content(&s.content); set_gutter_status(s.drive_id.is_none()); } } resize_editor(); focus_editor(); }).forget(); 
                }
            });
        })
    };
    ...
    let on_fallback_cfm = {
        let f_queue = fallback_queue.clone();
        let s_state = sheets.clone();
        let r_s = sheets_ref.clone();
    ...
    let on_delete_category_cfm = {
        ...
        let s_state = sheets.clone();
        let r_s = sheets_ref.clone();
        let ncid_state = no_category_folder_id.clone();

        Callback::from(move |_: usize| {
            if let Some(target_cat_id) = (*pending).clone() {
                let pending = pending.clone(); let il = il.clone(); let ifo = ifo.clone();
                let lmk = lmk.clone(); let on_refresh = on_refresh.clone();
                let s_state = s_state.clone(); let r_s = r_s.clone(); let ncid_state = ncid_state.clone();
                lmk.set("synchronizing"); il.set(true); ifo.set(false);
                spawn_local(async move {
                    let structure = match ensure_directory_structure().await { Ok(res) => res, Err(_) => { return; } };
                    let no_cat_id = js_sys::Reflect::get(&structure, &JsValue::from_str("noCategoryId")).unwrap().as_string().unwrap();
                    ncid_state.set(Some(no_cat_id.clone()));
                    if let Ok(files_res) = crate::drive_interop::list_files(&target_cat_id, None).await {
                        if let Ok(files_val) = js_sys::Reflect::get(&files_res, &JsValue::from_str("files")) {
                            let files_arr = js_sys::Array::from(&files_val);
                            for i in 0..files_arr.length() {
                                let f_meta = files_arr.get(i);
                                let f_id = js_sys::Reflect::get(&f_meta, &JsValue::from_str("id")).unwrap().as_string().unwrap();
                                let _ = crate::drive_interop::move_file(&f_id, &target_cat_id, &no_cat_id).await;
                            }
                        }
                    }
                    let _ = crate::drive_interop::delete_file(&target_cat_id).await;
                    let mut us = (*s_state).clone(); let mut changed = false;
                    for s in us.iter_mut() { if s.category == target_cat_id { s.category = no_cat_id.clone(); changed = true; } }
                    if changed {
                        let ser = serde_wasm_bindgen::Serializer::json_compatible();
                        for s in us.iter() {
                            let js = JSSheet { id: s.id.clone(), guid: s.guid.clone(), category: s.category.clone(), title: s.title.clone(), content: s.content.clone(), is_modified: s.is_modified, drive_id: s.drive_id.clone(), temp_content: s.temp_content.clone(), temp_timestamp: s.temp_timestamp, last_sync_timestamp: s.last_sync_timestamp, tab_color: s.tab_color.clone() };
                    ...
                    pending.set(None); on_refresh.emit(()); ifo.set(true); Timeout::new(300, move || { il.set(false); }).forget();
                });
            }
        })
    };
    ...
    let on_import_cb = {
        ...
        let ncid = no_category_folder_id.clone();
        Callback::from(move |_| {
            let s_state = s_state.clone(); let aid_state = aid_state.clone();
            let sp_state = sp_state.clone(); let r_s = r_s.clone();
            let cat_id = (*ncid).clone().unwrap_or_else(|| "NO_CATEGORY".to_string());
            ...
            let on_change = Closure::wrap(Box::new(move |_: web_sys::Event| {
                if let Some(files) = input_c.files() {
                    if let Some(file) = files.get(0) {
                        ...
                        let on_load = {
                            let filename = filename.clone(); let s_state = s_state.clone();
                            let aid_state = aid_state.clone(); let sp_state = sp_state.clone();
                            let r_s = r_s.clone(); let reader_c = reader_c.clone();
                            ...
                            Closure::wrap(Box::new(move |_: web_sys::Event| {
                                if let Ok(content) = reader_c.result() {
                                    if let Some(text) = content.as_string() {
                                        ...
                                        let ns = Sheet { id: nid.clone(), guid: None, category: cat_id_for_sheet.clone(), title: filename.clone(), content: text.clone(), is_modified: true, drive_id: None, temp_content: Some(text.clone()), temp_timestamp: Some(js_sys::Date::now() as u64), last_sync_timestamp: None, tab_color: generate_random_color() };
                                        ...
                                        spawn_local(async move {
                                            let js = JSSheet { id: nid, guid: None, category: cat_id_for_final, title: filename, content: text, is_modified: true, drive_id: None, temp_content: None, temp_timestamp: None, last_sync_timestamp: None, tab_color: ns.tab_color };
                                            let ser = serde_wasm_bindgen::Serializer::json_compatible();
                                            if let Ok(v) = js.serialize(&ser) { let _ = save_sheet(v).await; }
                                        });
                                    }
    ...
    // Editor Initialization
    {
        ...
        let sp_init = is_suppressing_changes.clone(); let r_s = sheets_ref.clone(); let r_aid = active_id_ref.clone();
                ...
                let db_ready = *db_loaded;
                use_effect_with((is_auth, ncid.clone(), is_ld, db_ready), move |deps| {
                    let (auth, _, loading, ready) = deps;
                    if *auth && !*loading && *ready {
                        let os_i = os.clone(); let on_i = on.clone(); let oo_i = oo.clone(); 
         let oi_i = oi.clone();
                        ...
                        if let Some(id) = aid {
                            ...
                            if let Some(sheet) = cur_s.iter_mut().find(|s| s.id == id) {
                                if sheet.content != cur_c {
                                    ...
                                    if !sheet.is_modified { sheet.is_modified = true; }
                                    needs_upd = true;
                                }
    ...
    let conflict_dialog = if let Some(conflict) = conflict_queue.first() {
        ...
        let cq = conflict_queue.clone();
        Some(html! { <CustomDialog title={title} message={message} options={options} on_confirm={on_conf_cfm} on_cancel={let cq = cq.clone(); Some(Callback::from(move |_| { cq.set(Vec::new()); }))} on_start_processing={let il = is_loading.clone(); let ifo = is_fading_out.clone(); let lmk = lmk_conflict.clone(); move |_| { lmk.set("saving"); il.set(true); ifo.set(false); }} /> })
    } else { None };
    ...
    let delete_category_dialog = if let Some(_) = *pending_delete_category {
        let title = i18n::t("delete", lang);
        let message = i18n::t("confirm_delete_category", lang);
        let pending = pending_delete_category.clone();
        let on_cfm = on_delete_category_cfm.clone();
    ...
    let fallback_alert = if let Some(_) = fallback_queue.first() {
        let fq = fallback_queue.clone();
        Some(html! { <CustomDialog title={i18n::t("category_not_found_title", lang)} message={i18n::t("category_not_found_fallback", lang)} options={vec![DialogOption { id: 0, label: "OK".to_string() }]} on_confirm={on_fallback_cfm} on_cancel={let fq = fq.clone(); Some(Callback::from(move |_| { fq.set(Vec::new()); }))} /> })
    } else { None };

    let preview_overlay = if *is_preview_visible {
        let active_id = (*active_sheet_id).clone();
        let content = if let Some(id) = active_id { sheets.iter().find(|s| s.id == id).map(|s| s.content.clone()).unwrap_or_default() } else { "".to_string() };
    ...
    let main_ui = html! {
        <main key="main-editor-surface" class={classes!("absolute", "inset-0", "flex", "flex-col", "text-white", "transition-opacity", "duration-300", if !*is_authenticated { "opacity-0 invisible" } else { "opacity-100 visible" } )}>
            <ButtonBar key="top-button-bar" vim_mode={*vim_mode} on_toggle_vim={on_toggle_vim} on_new_sheet={on_new_sheet_cb.clone()} on_open={on_open_dialog} on_import={on_import_cb} on_change_font_size={on_change_font_size} on_change_category={on_change_category_cb} current_category={current_cat} categories={(*categories).clone()} />
                ...
                if !*is_authenticated {
                    <div class="pointer-events-auto fixed inset-0 flex items-center justify-center bg-gray-900 overflow-y-auto p-4">
                        <div class="text-center max-w-2xl">
                            ...
                            <div class="mb-10 text-gray-300 text-sm leading-relaxed whitespace-pre-wrap opacity-80 bg-gray-800/30 p-6 rounded-lg border border-white/5 shadow-inner">{ i18n::t("app_policy_description", lang) }</div>
                ...
                if *is_file_open_dialog_visible {
                    if let Some(ldid) = (*leaf_data_folder_id).clone() {
                        <div class="pointer-events-auto">
                            <FileOpenDialog on_close={let iv = is_file_open_dialog_visible.clone(); let sp = is_suppressing_changes.clone(); move |_| { iv.set(false); sp.set(false); focus_editor(); }} on_select={on_file_sel} leaf_data_id={ldid} categories={(*categories).clone()} on_refresh={on_refresh_cats_cb.clone()} on_delete_category={on_delete_category_cb} on_start_processing={let il = is_loading.clone(); let ifo = is_fading_out.clone(); let lmk = loading_message_key.clone(); move |_| { lmk.set("synchronizing"); il.set(true); ifo.set(false); }} />
                        </div>
                    }
                }
                if let Some(preview) = preview_overlay { <div class="pointer-events-auto">{ preview }</div> }
                if let Some(del_diag) = delete_category_dialog { <div class="pointer-events-auto">{ del_diag }</div> }
                if let Some(conf_diag) = conflict_dialog { <div class="pointer-events-auto">{ conf_diag }</div> }
                if let Some(fb_alert) = fallback_alert { <div class="pointer-events-auto">{ fb_alert }</div> }
                if *is_loading {
